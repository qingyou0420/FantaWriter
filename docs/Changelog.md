...
- **生文章质量（对照 `docs/迭代方案.md` P0 + 可干净做完的 P1）**：章节篇幅改为数字硬约束，不足自动续写一次并提供「一键补足」；本章可勾选出场人物，其余只注入一行名片，状态卡不再重复静态人设；原作焕新改为忠实事实、文笔跟指南，摘录按名称检索并随章节推进原文区段；学习文风的 fingerprints 写入生成提示词；卷 `Volume.summary` 进入记忆包，缺章摘要可一键补。另：按卷生成大纲、润色/续写带记忆与选区滑窗、正文后提示锁定冲突（不自动重写）、记忆包改为 assemble 直接拼接而不再 `user.replace`。
- **复查补丁**：生成请求 `mode` 保持字面量联合（不再被 `Record<string, unknown>` 推宽）；扩写倍数用数字 `2` 而不是字符串 `"2"`；润色/续写带上 `outline` 以免摘录锚到原稿结尾；自动续写同时传 `previousSummary` / `previousSummaries`；按卷生成大纲若该卷已有正文先确认。
