...
P1-5：深页排版——落笔行内收成「通过 + ⋯」、prompt 改对话框；研墨已定稿态与终局/伏笔分栏；织卷树底链接与筛选折叠；章页三键；正典卡文件名映射与 diff 折叠；短篇书房条对齐五页。不 bump。

P0-7：书内 chrome 上提到 App 顶栏（五页 + 章页 / 真相 / 数据分析同栏）；系统页面包屑删除、h1 统一 32 衬线；按钮三级；ConfirmDialog / Drawer 收口；短篇删除迁入设置危险区。不 bump。

P0-6：甲「墨 · 宣 · 朱」色板 + StageDot 圆点统一 + 文案删减（问心快捷芯片、首页落笔菜单、状态词一词一义）。不 bump，不改 appId / 安装包名。
