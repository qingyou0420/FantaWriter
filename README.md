...
当前版本：**2.0.11**。安装包：`FantaWriter-Setup-2.0.11.exe`（[Releases](https://github.com/qingyou0420/FantaWriter/releases)）。
...
产物在 `dist-installer/FantaWriter-Setup-2.0.11.exe`（另有 `Fantasy-Writer-Setup-2.0.11.exe` 别名）。打包前会预构建 Studio、`INKOS_DISABLE_VITE_BUILD=1`，把 `packages/studio/dist` + core 装进 extraResources，并拒绝把 `.env` / `secrets.json` 打进安装包。
