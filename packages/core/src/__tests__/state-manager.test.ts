...
describe("StateManager", () => {
  ...
  describe("acquireBookLock", () => {
    ...
    it("recovers an in-process lock whose heartbeat lease has expired", async () => {
      const { expireInProcessBookLockForTest } = await import("../state/manager.js");
      await mkdir(manager.bookDir("lock-book-process-ttl"), { recursive: true });
      const stale = await manager.acquireBookLock("lock-book-process-ttl");
      expect(expireInProcessBookLockForTest(tempDir, "lock-book-process-ttl")).toBe(true);

      const recovered = await manager.acquireBookLock("lock-book-process-ttl");
      expect(typeof recovered).toBe("function");
      await recovered();
      await stale();
    });

    it("force-releases an in-process lock without waiting for the holder", async () => {
      await mkdir(manager.bookDir("lock-book-force"), { recursive: true });
      const abort = new AbortController();
      const release = await manager.acquireBookLock("lock-book-force", {
        taskId: "task-hung",
        stage: "write",
        abort,
      });
      try {
        await expect(manager.acquireBookLock("lock-book-force")).rejects.toMatchObject({
          code: "BOOK_BUSY",
        });
        const busy = await manager.acquireBookLock("lock-book-force").catch((error) => error);
        expect(String(busy)).toContain("not recovered automatically");
        expect(String(busy)).not.toContain("Stale locks are recovered automatically");

        const released = await manager.forceReleaseBookLock("lock-book-force", { graceMs: 0 });
        expect(released?.taskId).toBe("task-hung");
        expect(released?.stage).toBe("write");
        expect(abort.signal.aborted).toBe(true);
        expect(manager.inspectBookLock("lock-book-force")).toBeNull();

        const next = await manager.acquireBookLock("lock-book-force");
        await next();
      } finally {
        await release();
      }
    ...
    it("carries holder task/stage on BOOK_BUSY owner info", async () => {
      await mkdir(manager.bookDir("lock-book-holder"), { recursive: true });
      const release = await manager.acquireBookLock("lock-book-holder", {
        taskId: "write-next-1",
        stage: "draft",
      });
      try {
        const info = manager.inspectBookLock("lock-book-holder");
