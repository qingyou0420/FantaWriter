...
describe("alias-legacy-setup names", () => {
  it("treats Inkborne-Setup as primary and emits both legacy prefixes", () => {
    ...
    const names = setupNamesForVersion("v2.1.1");
    expect(names.version).toBe("2.1.1");
    ...
    expect(names.legacy).toBe("FantaWriter-Setup-2.1.1.exe");
  });
});
...
describe("aliasLegacySetup", () => {
  it("copies primary Inkborne-Setup to both legacy names with matching sha256", () => {
    ...
    temps.push(distDir);
    const primaryName = "Inkborne-Setup-2.1.1.exe";
    ...
    const result = aliasLegacySetup({ distDir, version: "2.1.1" });
    expect(result.sha256).toBe(sha256);
    expect(result.aliases).toHaveLength(2);

    for (const name of ["FantaWriter-Setup-2.1.1.exe", "Fantasy-Writer-Setup-2.1.1.exe"]) {
  ...
  it("throws when the Inkborne primary is missing", () => {
    ...
    temps.push(distDir);
    expect(() => aliasLegacySetup({ distDir, version: "2.1.1" })).toThrow(/找不到安装包/);
  });
});
