...
describe("alias-legacy-setup names", () => {
  it("treats Inkborne-Setup as primary and emits both legacy prefixes", () => {
    ...
    const names = setupNamesForVersion("v2.1.6");
    expect(names.version).toBe("2.1.6");
    expect(names.primary).toBe("Inkborne-Setup-2.1.6.exe");
    expect(names.aliases).toEqual(["FantaWriter-Setup-2.1.6.exe", "Fantasy-Writer-Setup-2.1.6.exe"]);
    expect(names.legacy).toBe("FantaWriter-Setup-2.1.6.exe");
  });
});
...
describe("aliasLegacySetup", () => {
  it("copies primary Inkborne-Setup to both legacy names with matching sha256", () => {
    ...
    temps.push(distDir);
    const primaryName = "Inkborne-Setup-2.1.6.exe";
    ...
    const result = aliasLegacySetup({ distDir, version: "2.1.6" });
    expect(result.sha256).toBe(sha256);
    expect(result.aliases).toHaveLength(2);

    for (const name of ["FantaWriter-Setup-2.1.6.exe", "Fantasy-Writer-Setup-2.1.6.exe"]) {
  ...
  it("throws when the Inkborne primary is missing", () => {
    ...
    temps.push(distDir);
    expect(() => aliasLegacySetup({ distDir, version: "2.1.6" })).toThrow(/找不到安装包/);
  });
});
