...
describe("2.0.13 Windows installer contract", () => {
  it("keeps root and desktop on the same stable 2.0.13", () => {
    expect(rootPkg.version).toBe("2.0.13");
    expect(desktopPkg.version).toBe("2.0.13");
  ...
  it("names the NSIS artifact FantaWriter-Setup-2.0.13.exe", () => {
    expect(setupFileNameForVersion(rootPkg.version)).toBe("FantaWriter-Setup-2.0.13.exe");
