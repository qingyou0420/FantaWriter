...
describe("2.1.2 Windows installer contract", () => {
  it("keeps root and desktop on the same stable 2.1.2", () => {
    expect(rootPkg.version).toBe("2.1.2");
    expect(desktopPkg.version).toBe("2.1.2");
  ...
  it("names the NSIS artifact Inkborne-Setup-2.1.2.exe and keeps appId", () => {
    expect(setupFileNameForVersion(rootPkg.version)).toBe("Inkborne-Setup-2.1.2.exe");
  ...
  it("packs and releases Inkborne-Setup as primary, with both legacy aliases", () => {
    ...
    const names = setupNamesForVersion("2.1.2");
