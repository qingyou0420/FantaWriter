...
describe("2.0.14 Windows installer contract", () => {
  it("keeps root and desktop on the same stable 2.0.14", () => {
    expect(rootPkg.version).toBe("2.0.14");
    expect(desktopPkg.version).toBe("2.0.14");
  ...
  it("names the NSIS artifact FantaWriter-Setup-2.0.14.exe", () => {
    expect(setupFileNameForVersion(rootPkg.version)).toBe("FantaWriter-Setup-2.0.14.exe");
