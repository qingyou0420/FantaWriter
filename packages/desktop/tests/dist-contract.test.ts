...
describe("2.0.12 Windows installer contract", () => {
  it("keeps root and desktop on the same stable 2.0.12", () => {
    expect(rootPkg.version).toBe("2.0.12");
    expect(desktopPkg.version).toBe("2.0.12");
  ...
  it("names the NSIS artifact FantaWriter-Setup-2.0.12.exe", () => {
    expect(setupFileNameForVersion(rootPkg.version)).toBe("FantaWriter-Setup-2.0.12.exe");
