...
describe("versionFromSetupName (group 1 = semver)", () => {
  it("parses Inkborne-Setup-2.1.2.exe as 2.1.2", () => {
    const m = "Inkborne-Setup-2.1.2.exe".match(SETUP_RE);
    expect(m?.[1]).toBe("2.1.2");
    expect(versionFromSetupName("Inkborne-Setup-2.1.2.exe")).toBe("2.1.2");
  });
  ...
  it("parses pre-release suffix on all prefixes", () => {
    expect(versionFromSetupName("Inkborne-Setup-2.1.2-beta.1.exe")).toBe("2.1.2-beta.1");
...
describe("current installer filename", () => {
  it("builds Inkborne-Setup-${version}.exe", () => {
    ...
    expect(setupFileNameForVersion("2.1.2")).toBe("Inkborne-Setup-2.1.2.exe");
    expect(setupFileNameForVersion("v2.1.2")).toBe("Inkborne-Setup-2.1.2.exe");
    expect(versionFromSetupName(setupFileNameForVersion("2.1.2"))).toBe("2.1.2");
  });

  it("ranks Inkborne ahead of FantaWriter and the shipped 1.4.0 name", () => {
    expect(preferSetupRank("Inkborne-Setup-2.1.2.exe")).toBeLessThan(
      preferSetupRank("FantaWriter-Setup-2.1.2.exe")
    );
